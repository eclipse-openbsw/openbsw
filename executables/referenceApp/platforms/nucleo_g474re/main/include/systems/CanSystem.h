...
/**
 * \file CanSystem.h
 * \brief CAN system lifecycle component for the NUCLEO-G474RE platform (FDCAN).
 *
 * Provides the platform-specific CanSystem that manages a single FdCanTransceiver
 * instance on FDCAN1 and integrates with the openBSW lifecycle manager.
 */
...
/**
 * Platform-specific CAN system for the STM32G474RE NUCLEO board.
 *
 * Manages a single FdCanTransceiver on FDCAN1 (PA11 RX / PA12 TX, 500 kbit/s).
...
public:
    ...
    /**
     * Runs the CanSystem.
     * Enables CanRxRunnable, initializes and opens the FdCanTransceiver to set up
     * the FDCAN1 peripheral, configures NVIC priorities and enables FDCAN IRQs,
     ...
     * Closes and shuts down the FdCanTransceiver, disables CanRxRunnable, then
     ...
     * \return Pointer to the FdCanTransceiver if busId matches CAN_0, nullptr otherwise.
     */
