...
public:
     ...
     * \param initialValue
     */
    void init(mutex_t initialValue = 0U) { mutex_ = initialValue; }
...
public:
    ...
    constexpr QueueMutex()
    {
        ...
        static_assert(
            etl::is_pointer<T>::value && etl::is_integral<typename etl::types<T>::type>::value,
...
public:
    ...
    {
    public:
        explicit constexpr Receiver(Queue& queue) : queue_(queue) {}
         ...
         */
        constexpr uint32_t size() const { return queue_.size(); }
         ...
         */
        constexpr bool isEmpty() const { return queue_.isEmpty(); }
         ...
         */
        QueueItem const& peek() const { return queue_.buffer_[queue_.getReceived() % MAX_SIZE]; }
         ...
         *
         */
        void advance() { queue_.advanceReceived(); }
    ...
    {
    public:
        explicit constexpr Sender(Queue& queue) : queue_(queue) {}
         ...
         */
        constexpr bool isFull() const { return queue_.isFull(); }
        ...
        {
            LockStrategy const lock(queue_.mutex_.get());
            etl::optional<size_t> index = queue_.writeNext();
            bool res                    = index.has_value();
            if (res)
            {
                queue_.buffer_[index.value()] = value;
            }
...
private:
    etl::array<QueueItem, MAX_SIZE> buffer_;
    MutexType mutex_ __attribute__((aligned(4)));
};
