...
constexpr MemberId MAX_METHOD_ID = 128U;
constexpr ClusterId INVALID_CLUSTER_ID
    = static_cast<ClusterId>(etl::numeric_limits<etl::underlying_type_t<ClusterId>>::max());
constexpr ServiceId INVALID_SERVICE_ID      = etl::numeric_limits<ServiceId>::max();
constexpr InstanceId INVALID_INSTANCE_ID    = etl::numeric_limits<InstanceId>::max();
constexpr RequestId INVALID_REQUEST_ID      = etl::numeric_limits<RequestId>::max();
constexpr uint8_t INVALID_ADDRESS_ID        = etl::numeric_limits<uint8_t>::max();
constexpr uint16_t FUTURE_DISPATCHER_OFFSET = static_cast<uint16_t>(sizeof(uint32_t) * 8U);
constexpr uint32_t INVALID_TASK_ID          = etl::numeric_limits<uint32_t>::max();
constexpr uint16_t INVALID_MEMBER_ID        = etl::numeric_limits<uint16_t>::max();
