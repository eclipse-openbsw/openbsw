...
transport::TransportMessage buildResponsePendingTransportMessage(
    ...
    if (_nestedRequest != nullptr)
    {
        ...
        else if (!_nestedRequest->isPendingSent)
        {
            ...
            { // Only send ResponsePending while response is not being sent
                ...
                {
                    _pendingMessage = buildResponsePendingTransportMessage(
                        _pendingMessageBuffer, sourceAddress, responseSourceAddress, serviceId);
