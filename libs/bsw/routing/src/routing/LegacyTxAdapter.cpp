...
: LegacyTxAdapter(outputWriter, TxAdapterTable(), errorHandler)
{
    if (!load(config, _table))
    {
        logger::Logger::error(logger::ROUTING, "Failed to load TX adapter table from config");
    }
}
...
void LegacyTxAdapter::commit()
{
    ...
    if (bufferSize < 2 * sizeof(::etl::be_uint32_t))
    {
        logger::Logger::error(
            logger::ROUTING,
            "Buffer size %lu is too small to contain message ID and length",
    ...
    if (out.size() != messageBufferSize)
    {
        ...
        logger::Logger::error(
            logger::ROUTING,
            "Can't output message %x, length %ld",
            static_cast<uint32_t>(messageId),
    ...
    if (pduOffset >= messageLength)
    {
        logger::Logger::error(
            logger::ROUTING,
            "Invalid PDU offset. message = %x, message length = %ld, offset = %ld, pdu "
            "length = %ld\n",
            static_cast<uint32_t>(messageId),
            static_cast<uint32_t>(messageLength),
