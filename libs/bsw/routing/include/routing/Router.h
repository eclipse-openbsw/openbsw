    ...
    ::etl::span<::io::IWriter*> writers)
{
        ...
        || (::etl::find(writers.begin(), writers.end(), nullptr) != writers.end()))
    {
        ::util::logger::Logger::error(
            ::util::logger::ROUTING, "Unable to initialize router: invalid reader(s) or writer(s)");
        return;
    }
    ...
    {
        ::util::logger::Logger::error(
            ::util::logger::ROUTING, "Unable to initialize router: empty routing configuration");
        return;
    }
    ...
    if (table.destinations.size() != table.outputMessageIds.size())
    {
        ::util::logger::Logger::error(
            ::util::logger::ROUTING,
            "Unable to initialize router: inconsistent configuration (destinations.size() != "
            "outputMessageIds.size())");
        return;
    }
...
bool Router<MAX_NUM_CHANNELS>::run()
{
    if (!_initialized)
    {
        ::util::logger::Logger::error(::util::logger::ROUTING, "Can't run router, not initialized");
        return false;
    }
    ...
    for (size_t i = 0; i < _readers.size(); ++i)
    {
        bool const pduRouted
            = route(_currentChannelId, _table, *_readers[_currentChannelId], _writers);
