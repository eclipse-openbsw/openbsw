...
public:
    ...
    /**
     * Construct a PduTransportRxAdapter from the provided config byte array.
     */
    PduTransportRxAdapter(
        ::io::IReader& inputReader, ::etl::span<uint8_t const> config, ErrorHandler errorHandler);
...
: RxAdapter(inputReader, _pduBuffer, config, errorHandler)
, _currentFrame()
, _currentMessageSize()
, _invalidMessageLengthPdus()
{}
