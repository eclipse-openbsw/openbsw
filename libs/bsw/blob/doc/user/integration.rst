...
'config's using the class's custom iterator implementation. However, it is recommended that the blob
span returned by ``load()`` be used for any further processing in a given module, and that the ``Blob``
iterator be constructed only on demand.