...
Config Config::from_bytes(::etl::span<uint8_t const> const mem)
{
    ...
    if (header.configType == Type::UNKNOWN)
    {
        ::util::logger::Logger::error(
            ::util::logger::BLOB,
            "Invalid type in config: %u\n",
            static_cast<uint32_t>(header.configType));
        return {};
    }

    if (header.size > ::etl::numeric_limits<uint32_t>::max() - sizeof(header))
    {
        ::util::logger::Logger::error(::util::logger::BLOB, "Header is too big\n");
        return {};
    }
    ...
    if (mem.size() < size)
    {
        ::util::logger::Logger::error(::util::logger::BLOB, "Invalid config size\n");
        return {};
    }
