...
public:
     ...
     * \param clusterId
     */
    void setTargetClusterId(uint8_t const clusterId) { header_.tgtClusterId = clusterId; }
     ...
     */
    bool isRequest() const { return hasActiveFlag_(Flags::Request); }
     ...
     */
    bool isFireAndForgetRequest() const { return hasActiveFlag_(Flags::FireAndForgetRequest); }
     ...
     */
    bool isResponse() const { return hasActiveFlag_(Flags::Response); }
     ...
     */
    bool isError() const { return hasActiveFlag_(Flags::Error); }
     ...
     */
    bool isEvent() const { return hasActiveFlag_(Flags::Event); }
     ...
     */
    bool hasUniqueExternalPayload() const { return hasActiveFlag_(Flags::UniqueExternalPayload); }
     ...
     */
    bool hasSharedExternalPayload() const { return hasActiveFlag_(Flags::SharedExternalPayload); }
     ...
     * ErrorState::NoError.
     */
    ErrorState getErrorState() const { return isError() ? payload_.error : ErrorState::NoError; }

private:
    friend class MessageAllocator;

    constexpr Message(Header const& header) : header_(header), payload_() {}
    ...
    constexpr T const& getObjectStoredInPayload_() const
    {
        ...
        return *reinterpret_cast<T const*>(payload_.internalBuffer.data());
    }
    ...
    constexpr void constructObjectAtPayload_(T const& obj)
    {
        ...
        unsetFlag_(Flags::UniqueExternalPayload);
        unsetFlag_(Flags::SharedExternalPayload);
     ...
     * \param isShared
     */
    void setExternalHandle_(ptrdiff_t const offset, size_t const size, bool const isShared)
    {
        setFlag_(isShared ? Flags::SharedExternalPayload : Flags::UniqueExternalPayload);
        etl::construct_object_at<ExternalHandle>(
            &payload_.externalHandle, ExternalHandle{offset, size});
    }
    ...
    {
        return (header_.flags & static_cast<uint8_t>(flag)) == static_cast<uint8_t>(flag);
    }
     ...
     * \param flag
     */
    constexpr void setFlag_(Flags const flag) { header_.flags |= static_cast<uint8_t>(flag); }
     ...
     * \param flag
     */
    constexpr void unsetFlag_(Flags const& flag) { header_.flags &= ~static_cast<uint8_t>(flag); }
