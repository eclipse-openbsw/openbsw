...
struct AdcStateTracker
{
    ...
    static void clearAdcalAfterWrite(uint32_t volatile& cr, uint32_t adcalBit) { cr &= ~adcalBit; }

    /// After writing ADEN=1 to CR, hardware sets ADRDY in ISR.
    static void setAdrdyAfterEnable(
        uint32_t volatile& cr, uint32_t volatile& isr, uint32_t adenBit, uint32_t adrdyBit)
    {
        if ((cr & adenBit) != 0U)
        {
            isr |= adrdyBit;
    ...
    static void setEocAfterStart(
        uint32_t volatile& cr, uint32_t volatile& isr, uint32_t adstartBit, uint32_t eocBit)
    {
        if ((cr & adstartBit) != 0U)
        {
            isr |= eocBit;
