...
bool FdCanDevice::transmit(::can::CANFrame const& frame)
{
    ...
    static constexpr uint32_t TX_ELEMENT_SIZE = 72U; // 18 words x 4 bytes
    uintptr_t ramBase                         = getInstanceRamBase(fdcan);
    uint32_t* txBuf
        = reinterpret_cast<uint32_t*>(ramBase + TX_BUFFER_OFFSET + (putIdx * TX_ELEMENT_SIZE));
...
uint8_t FdCanDevice::receiveISR(uint8_t const* filterBitField)
{
    ...
    while (toDrain > 0U)
    {
        ...
        // STM32G4 FDCAN uses 18-word (72-byte) element spacing in message RAM.
        static constexpr uint32_t RX_ELEMENT_SIZE = 72U; // 18 words x 4 bytes
        uint32_t const* rxBuf                     = reinterpret_cast<uint32_t const*>(
            ramBase + RX_FIFO0_OFFSET + (getIdx * RX_ELEMENT_SIZE));
