...
bsp::BspReturnCode FlashEepromDriver::erasePage(uintptr_t pageAddr)
{
    ...
    FLASH->CR = (FLASH->CR & ~(FLASH_CR_PNB_Msk)) | (pageNum << FLASH_CR_PNB_Pos) | FLASH_CR_PER;
...
FlashEepromDriver::programPage(uintptr_t destAddr, uint8_t const* data, uint32_t length)
{
...
#if defined(STM32G474xx)
    // STM32G4 flash requires double-word (64-bit) programming
    ...
    FLASH->CR &= ~FLASH_CR_PG;
#elif defined(STM32F413xx)
    // STM32F4: word (32-bit) programming, PSIZE = x32
    FLASH->CR |= FLASH_CR_PG;
