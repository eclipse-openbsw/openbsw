...
current CMake configurations. Pull requests, merge queue runs, and non-main branch pushes restrict clang-tidy to the
changed scope, while pushes to ``main`` keep the full repository scan.

For the branch-scoped CI runs, the workflow first fetches the relevant base and head commits for
``pull_request`` and ``merge_group`` events before it determines the changed source files. This keeps
...
This means the branch-mode clang-tidy flow is:

#. checkout with full history
#. fetch the relevant comparison commits for PR or merge queue runs
...
Pushes to ``main`` skip this reduced-scope path and continue to run the full repository scan.
